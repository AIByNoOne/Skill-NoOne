# as400-quality (IBM Bob)

Instruccion para analizar la calidad del codigo AS/400 (IBM i) estilo SonarQube, 100% local.
Usa el `AGENTS.md` de la raiz del repo, o copia este archivo a la convencion de tu version de Bob.
...
Exit code 1 si el quality gate da FAIL (hay BLOCKER/CRITICAL).
...
- Verificá `%EOF` tras READ, `%FOUND` tras CHAIN, variables sin inicializar, division por
  ...
  hardcodeadas, duplicacion y complejidad.
...
El entregable es un reporte consolidado: quality gate, conteo Bug/Vulnerability/Smell,
metricas por archivo y recomendaciones accionables (archivo:linea + fix). Correr solo el
script es resultado parcial.
